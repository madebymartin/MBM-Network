...
/*	$day = 'Thursday';
	...
	elseif($day === 'Friday' && $time > '11' || $day === 'Saturday' || $day === 'Sunday'){
		...
		$dispatch_message = 'It will be dispatched on Monday so You should receive your delivery on ' . date('l', strtotime('next tuesday')) . '.';
	...
	elseif($time < '12'){
		...
		$dispatch_message = 'It will be dispatched today so you should receive your delivery tomorrow.';
		$deldate = date("l jS F", $date_timestamp + '86400');
	}
	else{
		// MON - WEDS AFTER dispatch
		$dispatch_message = 'It will be dispatched tomorrow so you should receive your delivery on ' . date("l", $date_timestamp + '172800');
