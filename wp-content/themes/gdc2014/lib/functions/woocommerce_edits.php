...
add_action( 'woocommerce_admin_order_item_headers', 'display_skx', 10, 1);
function display_skx($order){
    $order_id = $order->id;
    $skx_recommendations = get_post_meta( $order_id, 'skx_recommendations', true );
...
function gdc_custom_cart_button_archive() {
    ...
    $button_text = 'Add to bag (£' . $prodprice . ')';
        return __( $button_text, 'woocommerce' );   
}
...
//function mbm_woocommerce_template_single_rating(){}
...
function mbm_wishlist_guest_link(){
    if ( is_user_logged_in() ) { }
    else{
        $url = get_permalink();
        echo '<div class="tryasample"><a href="https://germaine-de-capuccini.co.uk/my-account?rd=' . $url . '">login or register to add this to a wishlist</a></div>';
    }
}
add_action( 'woocommerce_single_product_summary', 'mbm_wishlist_guest_link', 30 );
...
// REMOVE REVIEWS TAB - NOW HANDLED BY FEEFO
...
function product_instructions_tab_content( $tabs ) { 
    if ( get_post_meta(get_the_ID(), "_cmb_instructions", true) ) {
        $content = get_post_meta(get_the_ID(), "_cmb_instructions", true);
        echo '<h2>How to use ' . get_the_title(get_the_ID()) . '</h2>';
...
function product_ingredients_tab_content() { 
    if ( get_post_meta(get_the_ID(), "_cmb_ingredients", true) ) {
        $content = get_post_meta(get_the_ID(), "_cmb_ingredients", true);
        echo '<h2>' . get_the_title(get_the_ID()) . ' Ingredients</h2>';
        echo wpautop( $content, true );
...
function product_sticky_nav(){
	if(is_product()){

        global $product;
        // Feefo Tab
        $sku = $product->get_sku();
        $xml_url = "http://ww2.feefo.com/api/xmlfeedback?merchantidentifier=germaine-de-capuccini&protocol=https&mode=product&negativesanswered=true&sortby=date&order=asc&allcomments=True&since=year&vendorref=" . $sku;
		...
		$nav_list ='';
		$nav_list .= '<li><a href="#wrapper">Top</a></li>';
		if( get_the_content()){ $nav_list .= '<li><a href="#section-description">Details</a></li>';}
		if( get_post_meta(get_the_ID(), "_cmb_instructions", true) ) {$nav_list .= '<li><a href="#section-instructions_tab">Instructions</a></li>';}
		if( get_post_meta(get_the_ID(), "_cmb_ingredients", true) ) {$nav_list .= '<li><a href="#section-ingredients_tab">Ingredients</a></li>';}
        if( $review_count > 0 ){ $nav_list .= '<li><a href="#section-feeforeviews">Reviews</a></li>';}
		echo '<nav class="mobilehide" id="product_sticky_nav">';
			echo '<ul>';
				echo $nav_list;
			echo '</ul>';
		echo '</nav>';
...
add_action('thematic_before', 'product_sticky_nav', 0);



add_action( 'woocommerce_single_product_summary', 'insert_product_size', 22 );
function insert_product_size(){
    $product_size = get_post_meta( get_the_ID(), '_cmb_size', true );
    if($product_size){ echo '<p class="size">(' . $product_size . 'ml.)</p>'; }      
}



add_action( 'woocommerce_single_product_summary', 'sample_full_size_link', 30 );
function sample_full_size_link(){
    //global $woocommerce;
    $sample_id = get_post_meta(get_the_ID(), 'sample_version', true);
    if(get_post_meta( $sample_id, '_cmb_size', true )){ $sample_size = get_post_meta( $sample_id, '_cmb_size', true ) . 'ml.'; } else { $sample_size = ''; }
    $page_url = rtrim(get_permalink(),'/');
    $add_to_cart_url = $page_url . '/?add-to-cart=' . $sample_id . '' ;
    $product = new WC_Product( $sample_id );
    $price_html = $product->get_price_including_tax();
    $sample_price = woocommerce_price($price_html);
    //$sample_img = $product->get_image();
    //$cart_contents = $woocommerce->cart->get_cart();
    if($sample_id){ echo '<div class="tryasample">Try a '. $sample_size .' sample for ' . $sample_price . ' <a class="" href="' . $add_to_cart_url . '">Buy sample</a></div><br>'; }
}
...
add_action('woocommerce_before_shop_loop', 'insert_product_term_description');
function insert_product_term_description(){
