...
<head>
 ...
 <body>
     <div itemtype="https://schema.org/Service" itemscope="itemscope">
     <!-- tr class=row -->
           
               <xsl:for-each select="FEEDBACKLIST/SUMMARY">
                       <div class="woocommerce-product-rating">
<!--                            <p><span itemprop="ratingValue"><xsl:attribute name="content"><xsl:value-of select="AVERAGE"/>%</xsl:attribute><xsl:value-of select="AVERAGE"/>% of customers are happy with the service they received from <xsl:value-of select="TITLE"/> (<xsl:value-of select="TOTALSERVICECOUNT"/> reviews).</span></p> -->

                           <p><a class="newtab" target="blank" href="https://ww2.feefo.com/en-gb/reviews/germaine-de-capuccini#?timeFrame=ALL">View all our reviews on Feefo</a></p>
                   ...
                   <xsl:for-each select="FEEDBACKLIST/FEEDBACK">
                       ...
                       <tr itemtype="https://schema.org/Review" itemscope="itemscope" itemprop="review">
                           ...
                           <td>
                               ...
                               <p itemprop="description"><xsl:value-of select="CUSTOMERCOMMENT" disable-output-escaping="yes"/></p>
                               <xsl:if test="VENDORCOMMENT"></xsl:if>
                               ...
                               <xsl:for-each select="FURTHERCOMMENTSTHREAD/POST">
                                   ...
                                   <xsl:if test="VENDORCOMMENT">
                                       <blockquote class="triangle-isosceles"><xsl:value-of select="VENDORCOMMENT" disable-output-escaping="yes" /></blockquote>
                                   </xsl:if>
                               </xsl:for-each>
