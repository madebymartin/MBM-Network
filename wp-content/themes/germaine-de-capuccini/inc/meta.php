...
add_action( 'cmb2_admin_init', 'mbm_register_front_page_repeatable_banners' );
/**
 * Hook in and add a metabox to demonstrate repeatable grouped fields
 */
function mbm_register_front_page_repeatable_banners() {
	$prefix = 'mbm_';

	/**
	 * Repeatable Field Groups
	 */
	$cmb_front_page_banners_group = new_cmb2_box( array(
		'id'           => $prefix . 'banners_group',
	...
	$group_field_id = $cmb_front_page_banners_group->add_field( array(
		'id'          => $prefix . 'banners',
		'type'        => 'group',
		'description' => __( '<b>Banner 1</b> is full width, others are dynamically sized, depending on how many are added', 'cmb2' ),
		'options'     => array(
			'group_title'   => __( 'Banner {#}', 'cmb2' ), // {#} gets replaced by row number
	...
	$cmb_front_page_banners_group->add_group_field( $group_field_id, array(
		'name' => __( 'Background Image', 'cmb2' ),
		'id'   => $prefix . 'bg_img',
		'type' => 'file',
	) );
	$cmb_front_page_banners_group->add_group_field( $group_field_id, array(
		'name' => __( 'Foreground Image', 'cmb2' ),
		'description' => __( 'Use transparent png', 'cmb2' ),
		'id'   => $prefix . 'fg_img',
		'type' => 'file',
	) );
	$cmb_front_page_banners_group->add_group_field( $group_field_id, array(
	...
	$cmb_front_page_banners_group->add_group_field( $group_field_id, array(
		...
		'id'      => $prefix . 'text_colour',
		'type'    => 'colorpicker',
		'default' => '#ffffff',
		'attributes' => array(
			'data-colorpicker' => json_encode( array(
				'palettes' => array( '#ffffff', '#ff834c', '#4fa2c0', '#000000', ),
			) ),
		),
	) );
	$cmb_front_page_banners_group->add_group_field( $group_field_id, array(
 ...
 */
add_action( 'cmb2_admin_init', 'mbm_register_product_meta' );
function mbm_register_product_meta() {
    ...
    $samples = new WP_Query($sample_args);
    $sample_choices = array();
	if($samples->have_posts()){
		while( $samples->have_posts() ) : $samples->the_post();
			$id = get_the_ID();
			$product = new WC_Product($id);
			$sku = $product->get_sku();
			$name = '(' . $sku . ') ' . get_the_title();
			$sample_choices[$id] = $name;
		endwhile;
	}
	asort($sample_choices);
    ...
    $treatments = new WP_Query($treatment_args);
    $treatment_choices = array();
	if($treatments->have_posts()){
		while( $treatments->have_posts() ) : $treatments->the_post();
			$id = get_the_ID();
			$name = get_the_title();
			$treatment_choices[$id] = $name;
		endwhile;
	}
	...
	$cmb_product_info->add_field( array(
		'name' => __( 'Size', 'cmb2' ),
		'desc' => __( '(include units eg ml)', 'cmb2' ),
		'id'   => $prefix . 'size',
		'type' => 'text',
	) );
	$cmb_product_info->add_field( array(
		'name' => __( 'Sample Version', 'cmb2' ),
		'id'   => $prefix . 'sample_id',
		'type'             => 'select',
	...
	$cmb_product_info->add_field( array(
		'name' => __( 'Key Points', 'cmb2' ),
		'id'   => $prefix . 'key_points',
		'type' => 'text',
		'repeatable' => true
	) );
	$cmb_product_info->add_field( array(
		'name' => __( 'Related Treatments', 'cmb2' ),
		'id'   => $prefix . 'related_treatment',
		'type'             => 'select',
		'show_option_none' => true,
		'options'          => $treatment_choices,
		'repeatable' => true
	) );
}
...
/**
 * Add Product Ingredients Meta Box
 */
add_action( 'cmb2_admin_init', 'mbm_register_product_ingredients_meta' );
function mbm_register_product_ingredients_meta() {
	$prefix = 'mbm_product_';

	$cmb_product_info = new_cmb2_box( array(
		'id'           => $prefix . 'further_info',
		'title'        => __( 'Further Information', 'cmb2' ),
		'object_types' => array( 'product', ), // Post type
		'context'      => 'normal',
		'priority'     => 'high',
		'show_names'   => true, // Show field names on the left
	...
	$cmb_product_info->add_field( array(
		'name' => __( 'Instructions for use', 'cmb2' ),
		'id'   => 'instructions',
		'type' => 'wysiwyg',
	) );
}
 ...
 * Add Treatment Meta
 */
add_action( 'cmb2_admin_init', 'mbm_register_treatment_meta' );
function mbm_register_treatment_meta() {
	$prefix = 'mbm_treatment_';

	$cmb_product_info = new_cmb2_box( array(
		'id'           => $prefix . 'spec',
		'title'        => __( 'Details', 'cmb2' ),
		'object_types' => array( 'treatment', ), // Post type
		'context'      => 'side',
		'priority'     => 'high',
		'show_names'   => true, // Show field names on the left
	) );
 ...
 * Add Treatment Meta
 */
add_action( 'cmb2_admin_init', 'mbm_register_salon_meta' );
function mbm_register_salon_meta() {
	$prefix = 'mbm_salon_';

	$account_managers = get_users( array(
		'blog_id'      => $GLOBALS['blog_id'],
		'role__in'         => array('staff', 'administrator', 'shop_manager'),
		'orderby'      => 'display_name',
		'order'        => 'ASC',
		) 
	);
	$account_manager_choices = array();
	foreach($account_managers as $account_manager){
    	$name = $account_manager->display_name;
    	$id = $account_manager->ID;
        $account_manager_choices[$id] = $name;
    }

	$cmb_salon_info = new_cmb2_box( array(
		'id'           => $prefix . 'details',
		'title'        => __( 'Details', 'cmb2' ),
		'object_types' => array( 'salon', ), // Post type
		'context'      => 'side',
		'priority'     => 'high',
		'show_names'   => true, // Show field names on the left
	) );

	$cmb_salon_info->add_field( array(
		'name' => __( 'Account Number', 'cmb2' ),
		'id'   => $prefix . 'account_number',
		'type' => 'text',
	) );

	$cmb_salon_info->add_field( array(
		'name' => __( 'Account Manager', 'cmb2' ),
		'id'   => $prefix . 'account_manager',
		'type'             => 'select',
		'show_option_none' => true,
		'options'          => $account_manager_choices,
	) );
...
// add_action( 'cmb2_admin_init', 'mbm_register_demo_metabox' );
/**
 * Hook in and add a demo metabox. Can only happen on the 'cmb2_admin_init' or 'cmb2_init' hook.
 */
function mbm_register_demo_metabox() {
	$prefix = 'mbm_';
	...
	$cmb_demo->add_field( array(
		...
		'id'         => $prefix . 'text',
		'type'       => 'text',
		'show_on_cb' => 'mbm_hide_if_no_cats', // function should return a bool value
	...
	$cmb_demo->add_field( array(
		...
		// );
		// 'display_cb' => 'mbm_display_text_small_column', // Output the display of the column values through a callback.
	) );
	...
	$cmb_demo->add_field( array(
		...
		'type' => 'text',
		'render_row_cb' => 'mbm_render_row_cb',
	) );
	...
	$cmb_demo->add_field( array(
		...
		'type'         => 'text',
		'before_row'   => 'mbm_before_row_if_2', // callback
