...
$dates = mbm_get_all_bookable_dates('', $location_id);
...
/*			if ( comments_open() || get_comments_number() ) :
		...
		echo '<div class="hentry">';
			...
			if(!empty($dates)){
				echo '<h2>The following is coming up soon in '. get_the_title($location_id) .'</h2>';
				...
				echo '<h2>Nothing currently due in '. get_the_title($location_id) .'</h2>';
			...
			$other_locations = new WP_Query( array (
			    'orderby'               => 'date',
			    'posts_per_page'        => -1,
			    'fields' => 'ids',
			    'post_type' => 'location',
			...
			if ( $other_locations->have_posts() ) {
				echo 'Also see  ';
				$other_location_links = array();
				while ( $other_locations->have_posts() ) : $other_locations->the_post();
					$other_location_links[] = '<a href="'. get_permalink(get_the_ID()) .'">'. get_the_post_thumbnail(get_the_ID(), "thumbnail", array( 'class' => 'circle smallcircle' )) . ' ' . get_the_title(get_the_ID()) .'</a>';
						//echo '<a href="'. get_permalink(get_the_ID()) .'">'. get_the_title(get_the_ID()) .'</a>';
				endwhile;
				// Prevent weirdness
				wp_reset_postdata();
				echo implode(', ', $other_location_links);
			}
