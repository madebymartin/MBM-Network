     ...
     <!-- tr class=row -->
               ...
               <xsl:for-each select="FEEDBACKLIST/SUMMARY">
                       <div class="woocommerce-product-rating">
                           <p><span><xsl:attribute name="content"><xsl:value-of select="AVERAGE"/>%</xsl:attribute><xsl:value-of select="AVERAGE"/>% of customers like <xsl:value-of select="TITLE"/> (<xsl:value-of select="TOTALSERVICECOUNT"/> reviews).</span></p>
                           <span><xsl:attribute name="content"><xsl:value-of select="BEST"/></xsl:attribute></span>
                           <span><xsl:attribute name="content"><xsl:value-of select="WORST"/></xsl:attribute></span>
                       </div>
               </xsl:for-each>

               <table class="comm-table">
                   <tr>
                       <th class="date" width="100">Reviewed</th>
                       <th class="score" width="20">Score</th>
                       <th class="comment">Customer's Feedback</th>
                   </tr>


                   <xsl:for-each select="FEEDBACKLIST/FEEDBACK">

                       <tr itemtype="https://schema.org/Review" itemscope="itemscope" itemprop="review">
                           ...
                           <td>
                            ...
                            <p>
                              <span itemprop="description"><xsl:value-of select="CUSTOMERCOMMENT" disable-output-escaping="yes"/></span>
                              <!-- <xsl:if test="PUBLICCUSTOMER">
                              </xsl:if> -->
                              <span class="hidden" itemprop="author" itemtype="https://schema.org/Person"> - <span itemprop="name">Happy Customer</span></span>
                            </p>

                               <xsl:if test="VENDORCOMMENT"><blockquote class="triangle-isosceles"><xsl:value-of select="VENDORCOMMENT" disable-output-escaping="yes" /></blockquote></xsl:if>
